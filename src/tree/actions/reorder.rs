...
pub fn reorder_element<'m1, 'm2: 'm1>(root: &mut NbtElement, indices: OwnedIndices, mapping: impl Into<Box<[usize]>>, bookmarks: &mut MarkedLines, mutable_indices: &'m1 mut MutableIndices<'m2>) -> Option<ReorderElementResult> {
	...
	let line_numbers = {
		...
		entries
			.iter()
			.map(|entry| {
				let new_line_number = current_line_number;
				current_line_number += entry.value.height();
				new_line_number
			})
			.collect::<Vec<_>>()
	};
	let true_line_numbers = {
		...
		entries
			...
			.map(|entry| {
				let new_line_number = current_line_number;
				current_line_number += entry.value.true_height();
				new_line_number
			})
			.collect::<Vec<_>>()
	};
	...
	let mut previous_entries = core::mem::take(entries)
		.into_iter()
		.map(|x| Some(x))
		.collect::<Vec<_>>();
