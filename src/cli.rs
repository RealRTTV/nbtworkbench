...
fn get_search_flags(args: &mut Vec<String>) -> SearchFlags {
	match get_argument_any(&["--search", "-s"], args).as_deref() {
...
fn get_exact_match(args: &mut Vec<String>) -> bool {
	get_argument_any(&["-em", "--exact-match"], args).is_some()
}

fn get_search_mode(args: &mut Vec<String>) -> SearchMode {
...
fn get_search_predicate(args: &mut Vec<String>) -> SearchPredicate {
	...
	let search_flags = get_search_flags(args);
	let exact_match = get_exact_match(args);
	let search_mode = get_search_mode(args);

	let inner = match (search_mode, exact_match) {
		(SearchMode::String, false) => SearchPredicateInner::StringCaseInsensitive(query),
		(SearchMode::String, true) => SearchPredicateInner::String(query),
		(SearchMode::Regex, exact_match) => match create_regex(query, exact_match) {
			Some(regex) => SearchPredicateInner::Regex(regex),
			None => {
				error!("Invalid regex, valid regexes look like: `/[0-9]+/g`");
				std::process::exit(1);
			},
		}
		(SearchMode::Snbt, exact_match) => match NbtElement::from_str(&query) {
			Ok((key, value)) => if exact_match { SearchPredicateInner::SnbtExactMatch((key, value)) } else { SearchPredicateInner::Snbt((key, value)) },
			Err(e) => {
	...
	SearchPredicate { search_flags, inner }
}
...
fn find_process(path: PathBuf, completed: &AtomicU64, total_size: u64, predicate: &SearchPredicate) -> Option<SearchResult> {
...
pub fn find() -> ! {
	...
	let results = std::thread::scope(|s| {
		...
		for p in paths {
			...
			results.push(s.spawn(move || find_process(path, completed, total_size, predicate)));
		}
...
fn replace_process(path: PathBuf, completed: &AtomicU64, total_size: u64, replacement: &SearchReplacement) -> Option<(PathBuf, usize)> {
	let bytes = match read(&path) {
		Ok(bytes) => bytes,
		Err(e) => {
			error!("File read error: {e}");
...
pub fn replace() -> ! {
	...
	let results = std::thread::scope(|s| {
		...
		for p in paths {
			...
			results.push(s.spawn(move || replace_process(path, completed, total_size, replacement)));
		}
		results.into_iter().filter_map(|x| x.join().ok()).filter_map(std::convert::identity).collect::<Vec<_>>()
	});
...
fn extension_and_format(args: &mut Vec<String>) -> (&'static str, NbtFileFormat) {
	let format_arg = get_argument_any(&["--format", "-f"], args);
	match format_arg.as_deref() {
		Some("nbt") => ("nbt", NbtFileFormat::Nbt),
		Some("dat" | "gzip") => ("dat", NbtFileFormat::Gzip),
		Some("dat_old") => ("dat_old", NbtFileFormat::Gzip),
		Some("zlib") => ("zlib", NbtFileFormat::Zlib),
		Some("snbt") => ("snbt", NbtFileFormat::Snbt),
		Some("lnbt") => ("nbt", NbtFileFormat::LittleEndianNbt),
		Some("lhnbt") => ("nbt", NbtFileFormat::LittleEndianHeaderNbt),
		None => {
...
fn reformat_process(path: PathBuf, completed: &AtomicU64, total_size: u64, format: NbtFileFormat, extension: &str, out_dir: &Path, child_path: PathBuf) {
	let bytes = match read(&path) {
		Ok(bytes) => bytes,
		Err(e) => {
			error!("File read error: {e}");
	...
	let mut new_path = out_dir.to_path_buf();
...
pub fn reformat() -> ! {
	...
	let (extension, format) = extension_and_format(&mut args);
	...
	let out_dir = out_dir.unwrap_or(root.clone());
	...
	std::thread::scope(|s| {
		for p in paths {
			...
			let out_dir = &*out_dir;
			s.spawn(move || reformat_process(pa, completed, total_size, format, extension, out_dir, p));
		}
	});
