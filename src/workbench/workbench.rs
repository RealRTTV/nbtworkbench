...
use crate::workbench::{ElementAction, FileFormat, MarkedLine, MarkedLines, PathWithName, Tab, TabConstants, WorkbenchAction};
...
impl Workbench {
		...
		match state {
			ElementState::Pressed => {
				...
				if let MouseButton::Left | MouseButton::Right = button {
					...
					if let Some(bulk) = WorkbenchAction::bulk(tab.selected_texts.iter_mut().filter_map(|selected_text| selected_text.write(consts, window_properties, &mut tab.value, &mut tab.path)).collect::<Vec<_>>()) {
...

				{
					...
					macro_rules! try_click_widget {
						($field:ident) => {
							if self.$field.is_clickable(&ctx.as_ref())
								&& self
									.$field
									.bounds((self.window_width, self.window_height).into())
									.contains((self.mouse_x, self.mouse_y).into())
							{
								// hardcoded to mouse_down only for now
								if self
									.$field
									.on_mouse_input(state, button, &mut ctx)
								{
									let ButtonWidgetAccumulatedResult {
										notifications,
										alerts,
										tabs,
										open_file_requests,
									} = ctx.take_accumulated();
					...
					match self.action_wheel.take() {
						...
						None => {
							if button == MouseButton::Right
								...
								&& depth + 1 == (self.mouse_x + horizontal_scroll - left_margin) / 16
							{
			...
			ElementState::Released => {
				if let MouseButton::Right = button {
					for selected_text in tab_mut!(self).selected_texts.iter_mut() {
	...
	fn process_action_wheel(&mut self) -> bool {
		...
		if (cy as isize - self.mouse_y as isize).pow(2) as usize + (cx as isize - self.mouse_x as isize).pow(2) as usize <= 8_usize.pow(2) {
			return true
		}
		...
		if let Some(action) = element.actions().get(highlight_idx).copied() && let Some(action) = action.apply(&mut tab.value, indices, &mut tab.bookmarks, mutable_indices!(tab), &mut self.alerts) {
		...
		for tab in &mut self.tabs {
			...
			match subscription.rx.try_recv() {
				Ok(data) => {
					...
					let action = replace_element(&mut tab.value, kv, subscription.indices.clone(), &mut tab.bookmarks, mutable_indices!(tab))
	...
	fn try_double_click_interaction(&mut self) -> bool {
		...
		if tab!(self).held_entry.is_some() || tab!(self).freehand_mode || now - tab!(self).last_selected_text_interaction.2 <= LINE_DOUBLE_CLICK_INTERVAL {
			return false
		};
	...
	fn try_steal(&mut self, can_initialize: bool) -> bool {
		...
		if tab!(self).held_entry.is_some() || tab!(self).freehand_mode {
			return false
		};
		let is_grid_layout = tab!(self)
			.value
			.as_region()
			.is_some_and(|region| region.is_grid_layout());
		...
		if let InteractionInformation::Content { is_in_left_margin: false, depth, x, y, .. } = get_interaction_information!(self) {
			if can_initialize {
				self.steal_animation_data
					.get_or_insert((now, (x, y).into()));
			}
	...
	fn steal(&mut self) -> bool {
		...
		if tab!(self).held_entry.is_some() {
			return false
		}
		let is_grid_layout = tab!(self)
			.value
			.as_region()
			.is_some_and(NbtRegion::is_grid_layout);
			...
			&& y > 0
		{
			...
			let RemoveElementResult { indices, kv: (key, mut value), replaces: _ } = match remove_element(&mut tab.value, indices, &mut tab.bookmarks, mutable_indices!(tab)) {
				Some(result) => result,
				None => {
	...
	fn duplicate(&mut self) -> bool {
			...
			&& y > 0
		{
			*indices.last_mut().expect("y > 0") += 1;
			...
			let tab = tab_mut!(self);
			let action = match add_element(&mut tab.value, (key, duplicate), indices, &mut tab.bookmarks, mutable_indices!(tab)) {
	...
	fn delete(&mut self, clipboard: bool) -> bool {
		...
		} = get_interaction_information!(self)
		{
			...
			let tab = tab_mut!(self);
			let result = match remove_element(&mut tab.value, indices, &mut tab.bookmarks, mutable_indices!(tab)) {
				Some(result) => result,
				None => {
					self.alert(Alert::error("Failed to remove element"));
					return false
	...
	fn drop_held_entry(&mut self) -> bool {
		...
		if self.mouse_x + horizontal_scroll + 16 < left_margin {
			return false
		}
		...
		if let Some(indices) = tab
			.value
			.create_drop_indices((kv.0.as_deref(), &kv.1), y, x)
		{
			if let Some(AddElementResult { indices, old_kv }) = add_element(&mut tab.value, kv, indices, &mut tab.bookmarks, mutable_indices!(tab)) {
					...
					Ok((key, element)) =>
						if element.id() == NbtChunk::ID && tab.value.id() != NbtRegion::ID {
							...
							let old_held_entry = tab
								.held_entry
								.replace(HeldEntry::from_aether((key, element)));
	...
	#[cfg(target_arch = "wasm32")]
	fn open_file(&mut self, _: &mut WindowProperties) { crate::wasm::try_open_dialog(); }

	#[must_use]
	#[deprecated = "use tab.left_margin() instead"]
	pub fn left_margin(&self) -> usize { tab!(self).left_margin() }

	#[must_use]
	#[deprecated = "use tab.scroll() instead"]
	pub fn scroll(&self) -> usize { tab!(self).scroll() }

	#[must_use]
	#[deprecated = "use tab.horizontal_scroll() instead"]
	pub fn horizontal_scroll(&self) -> usize { tab!(self).horizontal_scroll() }
	...
	fn try_select_search_box(&mut self, button: MouseButton) -> bool {
		if SearchBox::is_within_bounds((self.mouse_x, self.mouse_y), self.window_width) {
			self.search_box
				.select(self.mouse_x - SEARCH_BOX_START_X, button);
			self.replace_box.deselect();
	...
	fn try_select_text(&mut self, snap_to_ends: bool) -> bool {
		let tab = tab_mut!(self);
		let consts @ TabConstants { left_margin, horizontal_scroll, scroll } = tab.consts();
		...
		if let Some(selected_text) = SelectedText::for_y(consts, &mut tab.value, &mut tab.path, y, self.mouse_x, snap_to_ends, None) {
			tab.selected_texts.push(selected_text);
			true
		} else {
			false
		}
			...
			if let PhysicalKey::Code(key) = key.physical_key {
				...
				if self.search_box.is_selected() {
					match self.search_box.on_key_press(key, char, flags) {
						SearchBoxKeyResult::NoAction => {} // next thing, please
						SearchBoxKeyResult::GenericAction => {
							self.search_box
								.post_input((self.window_width, self.window_height));
							return true;
						}
						...
						SearchBoxKeyResult::MoveToReplaceBox => {
							self.search_box
								.post_input((self.window_width, self.window_height));
							self.replace_box.select(
								self.search_box
									.value
									.split_at(self.search_box.cursor)
									.0
									.width()
									.saturating_sub(self.search_box.horizontal_scroll),
								MouseButton::Left,
							);
							self.search_box.deselect();
							return true;
						...
						result @ (SearchBoxKeyResult::Search | SearchBoxKeyResult::SearchCountOnly) => {
							let notification = self
								.search_box
								.search(&mut tab.bookmarks, &tab.value, result == SearchBoxKeyResult::SearchCountOnly);
							self.notify(notification);
							self.search_box
								.post_input((self.window_width, self.window_height));
							return true;
					...
					match self.replace_box.on_key_press(key, char, flags) {
						ReplaceBoxKeyResult::NoAction => {} // next thing, please
						...
						ReplaceBoxKeyResult::MoveToSearchBox => {
							self.replace_box
								.post_input((self.window_width, self.window_height));
							self.search_box.select(
								self.replace_box
									.value
									.split_at(self.replace_box.cursor)
									.0
									.width()
									.saturating_sub(self.replace_box.horizontal_scroll),
								MouseButton::Left,
							);
							self.replace_box.deselect();
							return true;
						...
						ReplaceBoxKeyResult::ReplaceAll => {
							let (notification, bulk) = self
								.replace_box
								.replace(&mut tab.path, &mut tab.bookmarks, mutable_indices!(tab), &mut tab.value, &self.search_box);
				...
				}
				{
					...
					let mut completed_uuids = FxHashSet::default();
					while let Some(mut selected_text) = tab.selected_texts.iter().find(|text| !completed_uuids.contains(&text.uuid)).cloned() {
						let result = selected_text.on_key_press(key, char, flags, consts, window_properties, &mut tab.value, &mut tab.path, &mut tab.bookmarks, mutable_indices!(tab));
						let idx = tab.selected_texts.iter().find_position(|text| text.uuid == selected_text.uuid).map(|(idx, _)| idx);
						completed_uuids.insert(selected_text.uuid);
						if let &SelectedTextKeyResult::GenericAction | &SelectedTextKeyResult::Action(_) | &SelectedTextKeyResult::Escape | &SelectedTextKeyResult::Finish = &result {
							any_applied |= true;
							selected_text.post_input(consts);
						}
						match result {
							SelectedTextKeyResult::NoAction => {}
							SelectedTextKeyResult::GenericAction => {}
							SelectedTextKeyResult::Action(action) => {
								match action {
									Ok(Some(action)) => tab.append_to_history(action),
									Ok(None) => (),
									Err(e) => alerts.push(Alert::from(e)),
								}
							}
							SelectedTextKeyResult::Escape => {
								if let Some(idx) = idx {
									tab.selected_texts.remove(idx);
								}
								continue;
							}
							SelectedTextKeyResult::Finish => {
								if let Some(action) = selected_text.write(consts, window_properties, &mut tab.value, &mut tab.path) {
									tab.append_to_history(action);
									if let Some(idx) = idx {
										tab.selected_texts.remove(idx);
									}
						...
						if let Some(idx) = idx && let Some(slot) = tab.selected_texts.get_mut(idx) {
							*slot = selected_text;
						}
					}
					if any_applied {
				...
				if key == KeyCode::Escape
					&& flags == flags!()
					&& let Some(held_entry) = tab.held_entry.take()
				{
					tab.append_to_history(WorkbenchAction::DiscardHeldEntry { held_entry });
					return true;
				}
				if (key == KeyCode::Enter || key == KeyCode::NumpadEnter) && tab.selected_texts.is_empty() && flags == flags!() {
				...
				if key == KeyCode::KeyZ && flags == flags!(Ctrl) {
					if let Some(action) = tab.undos.pop() {
						match action
							.undo(&mut tab.value, &mut tab.bookmarks, mutable_indices!(tab), &mut tab.path, &mut tab.held_entry, window_properties)
							.context("Failed undo")
						{
	...
	pub fn try_extend_drag_selection(&mut self) {
		...
		if self.last_mouse_state == ElementState::Pressed {
			for selected_text in tab.selected_texts.iter_mut() {
				if tab.last_selected_text_interaction.1 == 0
					&& selected_text.is_drag_selectable()
				{
					let cursor = selected_text
						.selection
						.unwrap_or(selected_text.cursor);
					let selection = get_cursor_idx(
						&selected_text.value,
						(self.mouse_x + horizontal_scroll) as isize
							- (selected_text.indices.len() * 16 + 32 + 4 + left_margin) as isize
							- selected_text.prefix.0.width() as isize
							- selected_text
							.keyfix
							.as_ref()
							.map_or(0, |(a, _)| a.width()) as isize,
					);
			...
			if self.search_box.is_selected() && self.search_box.last_interaction.0 == 0 {
				let cursor = self
					.search_box
					.selection
					.unwrap_or(self.search_box.cursor);
	...
	fn set_tab(&mut self, idx: usize, window_properties: &mut WindowProperties) {
		...
		self.steal_animation_data = None;
		window_properties.set_window_title(format!("{} - NBT Workbench", tab!(self).path.name()).as_str());
	}
	...
	pub fn render(&mut self, builder: &mut VertexBufferBuilder) {
		if self.raw_window_width < MIN_WINDOW_WIDTH || self.raw_window_height < MIN_WINDOW_HEIGHT {
			return;
		}
		...
		let ghost = if self.mouse_x + horizontal_scroll >= left_margin && self.mouse_y >= HEADER_SIZE {
			tab.held_entry.as_ref().map(|entry| {
				(
					&entry.kv.1,
					Vec2u::new(((self.mouse_x + horizontal_scroll - left_margin) & !15) + left_margin, ((self.mouse_y - HEADER_SIZE) & !0b0111) + HEADER_SIZE),
				)
			})
		} else {
			None
		};
		let selected_text_y = tab
			.selected_text
			.as_ref()
			.map(|x| x.y)
			...
			if selected.keyfix.is_some() {
				...
				(
					selected
						.keyfix
						.clone()
						.map(|x| x.0.clone().into_boxed_str()),
					Some(selected.value.clone().into_boxed_str()),
					false,
				)
		...
		if mouse_y >= HEADER_SIZE && self.action_wheel.is_none() && !ReplaceBox::is_within_bounds((mouse_x, mouse_y), builder.window_width()) {
			builder.draw_texture_region_z((0, mouse_y & !15), BASE_Z, HOVERED_STRIPE_UV, (builder.window_width(), 16), (14, 14));
		}
		{
		...
		}
		{
			...
			tab.render(
				...
				self.steal_animation_data
					...
					.map(|x| {
						(now() - x.0)
							.min(LINE_DOUBLE_CLICK_INTERVAL)
							.as_millis() as f32 / LINE_DOUBLE_CLICK_INTERVAL.as_millis_f32()
					})
					.unwrap_or(0.0),
			);
			...
			macro_rules! render_button {
				($field:ident) => {
					if self.$field.is_visible(&ctx) {
						self.$field
							.render(builder, (self.mouse_x, self.mouse_y).into(), (self.window_width, self.window_height).into(), &ctx, &self.held_mouse_keys);
					}
				};
			}
	...
	pub fn tick(&mut self, window_properties: &mut WindowProperties) {
		#[cfg(not(target_arch = "wasm32"))]
		{
			...
			for (idx, tab) in self.tabs.iter_mut().enumerate() {
				if let Some(path) = tab.path.as_deref()
					&& path.is_absolute()
					&& (now() - tab.last_interaction >= Tab::AUTOSAVE_INTERVAL)
					&& tab.unsaved_changes
					&& tab.value.true_height() <= Tab::AUTOSAVE_MAXIMUM_LINES
				{
					if let Err(e) = tab.save(false, window_properties) {
						alerts.push(
							e.context(format!("Failed to autosave {nth} tab", nth = nth(idx + 1)))
								.into(),
						);
					}
				}
		...
		if (tab!(self).held_entry.is_some() || tab!(self).freehand_mode || ((tab!(self).selected_text.is_some() || self.search_box.is_selected()) && self.last_mouse_state == ElementState::Pressed))
		...
		if self.steal_animation_data.is_some() && self.try_steal(false) {
			if self
				.steal_animation_data
				.as_ref()
				.is_some_and(|x| (now() - x.0) >= LINE_DOUBLE_CLICK_INTERVAL)
			{
				self.steal();
			}
		} else {
		...
		for (idx, line) in lines.iter().enumerate() {
			...
			builder.settings(
				(
					...
					builder.window_height() - (idx + 1) * VertexBufferBuilder::CHAR_HEIGHT,
				),
				false,
	...
	fn render_tabs(&self, builder: &mut VertexBufferBuilder) {
		...
		for (idx, tab) in self.tabs.iter().enumerate() {
			...
			builder.draw_texture((offset - 32, 3), if tab.unsaved_changes { SAVE_UV } else { SAVE_GRAYSCALE_UV }, (16, 16));
	...
	fn render_action_wheel(&mut self, builder: &mut VertexBufferBuilder) {
		...
		if cy >= HEADER_SIZE {
			...
			} = Self::get_interaction_information_raw(left_margin, 0, scroll, cx, cy, &mut tab.value)
			else {
				return
			};
			let min_x = depth * 16 + left_margin;
			let max_x = min_x
				+ 32 + value.value_width()
				+ key
					.map(|key| key.width() + ": ".width())
					.unwrap_or(0);
	...
	pub fn try_search_box_scroll(&mut self) {
		...
		if self.last_mouse_state == ElementState::Pressed && self.search_box.is_selected() && !search_box_x.contains(&self.mouse_x) {
			if self.mouse_x < search_box_x.start {
				self.search_box.horizontal_scroll = self
					.search_box
					.horizontal_scroll
					.saturating_sub(4);
			} else {
				self.search_box.horizontal_scroll = (self.search_box.horizontal_scroll + 4).min(
					self.search_box
						.value
						.width()
						.saturating_sub(search_box_x.end - search_box_x.start),
				);
			}
		}
	}
	...
	pub fn try_replace_box_scroll(&mut self) {
		...
		if self.last_mouse_state == ElementState::Pressed && self.replace_box.is_selected() && !replace_box_x.contains(&self.mouse_x) {
			if self.mouse_x < replace_box_x.start {
				self.replace_box.horizontal_scroll = self
					.replace_box
					.horizontal_scroll
					.saturating_sub(4);
			} else {
				self.replace_box.horizontal_scroll = (self.replace_box.horizontal_scroll + 4).min(
					self.replace_box
						.value
						.width()
						.saturating_sub(replace_box_x.end - replace_box_x.start),
				);
			}
		}
	}

	pub fn should_ignore_event(&self) -> bool { now() < self.ignore_event_end }
