use crate::assets::{
	GZIP_FILE_TYPE_UV, HEADER_SIZE, HELD_SCROLLBAR_UV, LINE_NUMBER_SEPARATOR_UV, MCA_FILE_TYPE_UV, NBT_FILE_TYPE_UV, REDO_UV, SNBT_FILE_TYPE_UV, UNDO_UV, UNHELD_SCROLLBAR_UV, ZLIB_FILE_TYPE_UV,
};
...
impl Tab {
	#[must_use]
	pub fn new(nbt: NbtElement, file: File, path: &Path, compression: FileFormat, window_height: usize, window_width: usize) -> Option<Self> {
	...
	pub fn render(&self, builder: &mut VertexBufferBuilder, ctx: &mut RenderContext, held: bool, held_entry: Option<&NbtElement>) {
		...
		if builder.window_height() >= HEADER_SIZE {
			...
			if height > total {
				...
				if height > 2 {
					builder.draw_texture_region_z((builder.window_width() - 7, offset + 1), 0.0, uv + (0, 5), (6, height.saturating_sub(1)), (6, 4));
				}
				if height > 1 {
					builder.draw_texture((builder.window_width() - 7, offset + height), uv + (0, 15), (6, 1));
				}
			}
		}

		{
			...
			builder.draw_texture((builder.window_width() - 85, 27), LINE_NUMBER_SEPARATOR_UV + (0, 1), (2, 14));
			builder.draw_texture((builder.window_width() - 107, 26), LINE_NUMBER_SEPARATOR_UV, (2, 16));
			builder.draw_texture((builder.window_width() - 103, 26), UNDO_UV, (16, 16));
			let mut x = builder.window_width() - 82;
			for _ in 0..5_usize {
				if let Some(t) = tail {
		...
		{
			let mut tail = self.redos.tail.as_ref();
			builder.draw_texture((builder.window_width() - 191, 27), LINE_NUMBER_SEPARATOR_UV + (0, 1), (2, 14));
			builder.draw_texture((builder.window_width() - 213, 26), LINE_NUMBER_SEPARATOR_UV, (2, 16));
			builder.draw_texture((builder.window_width() - 209, 26), REDO_UV, (16, 16));
			let mut x = builder.window_width() - 188;
			for _ in 0..5_usize {
				if let Some(t) = tail {
					t.value.render(x, 26, builder, t.prev.is_none());
					x += 16;
