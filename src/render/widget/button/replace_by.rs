...
impl ButtonWidget for ReplaceByButton {
	...
	fn bounds(&self, window_dims: Vec2u) -> AxisAlignedBoundingBox { AxisAlignedBoundingBox::new(window_dims.x - SEARCH_BOX_END_X - 17, window_dims.x - SEARCH_BOX_END_X - 1, 50, 66) }
	...
	fn is_clickable(&self, ctx: &ButtonWidgetContext) -> bool { ctx.search_box.is_selected() || ctx.replace_box.is_selected() }

	fn is_visible(&self, ctx: &ButtonWidgetContext) -> bool { ctx.search_box.is_selected() || ctx.replace_box.is_selected() }
