...
impl SearchFlags {
	...
	#[must_use]
	pub fn has_key(self) -> bool { matches!(self, Self::Keys | Self::KeysValues) }
...
impl SearchMode {
	...
	#[must_use]
	pub fn has_exact_match_mode(&self) -> bool { matches!(self, Self::String | Self::Regex | Self::Snbt) }
...
impl SearchBox {
	...
	pub fn render(&self, builder: &mut VertexBufferBuilder) {
		...
		builder.draw_texture_region_z(pos, SEARCH_BOX_Z, DARK_STRIPE_UV, (builder.window_width() - SEARCH_BOX_END_X - pos.x, 22), (16, 16));
	...
	pub fn search(&self, bookmarks: &mut MarkedLines, root: &NbtElement, count_only: bool) -> Notification {
		...
		let new_bookmarks = if self.value.is_empty() {
			...
			let Some(predicate) = SearchPredicate::new(self.value.clone()) else {
				return Notification::new(format!("Invalid search syntax ({})", self.value), TextColor::Red, NotificationKind::Find)
			};
		...
		Notification::new(
			format!(
				"{hits} hit{s} for \"{arg}\" ({num_bookmarks} total bookmark{s2}) ({ms}ms)",
				s = if hits == 1 { "" } else { "s" },
				arg = self.value,
				ms = ms.as_millis(),
				num_bookmarks = bookmarks.len(),
				s2 = if bookmarks.len() == 1 { "" } else { "s" }
			),
			TextColor::White,
			NotificationKind::Find,
		)
	}
